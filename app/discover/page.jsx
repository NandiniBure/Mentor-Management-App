...
export default function UserDiscovery() {
	...
	useEffect(() => {
		...
		async function fetchNotifications() {
			try {
				const response = await fetch(
					`http://localhost:3000/api/notification?userId=${currentUser.id}`
				);
				...
				<div className='relative h-80 flex items-center justify-center bg-cover bg-center'>
					...
					<div className='absolute top-4 right-4 z-20 flex items-center gap-4'>
						...
						<div className='relative'>
							...
							{showNotifications && (
								<div className='absolute right-0 mt-2 w-96 bg-gray-800 text-white rounded-lg shadow-lg p-4'>
											...
											<button
												onClick={() => {
													if (
														...
														!currentUser.requested?.map((user) => user.userId)
															.includes(user.id)
													) {
